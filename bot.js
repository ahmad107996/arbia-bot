...
client.on('message', message => {//Toxic Codes
    if(message.content.startsWith(prefix + "toggleAntihack")) {//Toxic Codes
        ...
        if(!antihack[message.guild.id]) antihack[message.guild.id] = {//Toxic Codes
          onoff: 'Off'
        }//Toxic Codes
  ...
  data[ss.executor.id].time = 0
  u.guild.members.get(ss.executor.id).roles.forEach(r => {   //Toxic Codes
                  r.edit({
                      permissions : []  
                  });
                  data[ss.executor.id].time = 0
  ...
  client.on('channelDelete', (u) => {  //Toxic Codes
      u.guild.fetchAuditLogs().then( s => { //Toxic Codes
          var ss = s.entries.first();
          if (ss.action == `CHANNEL_DELETE`) { //Toxic Codes 
          if (!data[ss.executor.id]) {
              data[ss.executor.id] = { //Toxic Codes
              time : 1
            };
            if(antihack[u.guild.id].onoff === 'Off') return;
        } else {
